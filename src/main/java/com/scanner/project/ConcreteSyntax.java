...
public class ConcreteSyntax {
	...
	private Declarations declarations() {
		...
		while (token.getValue().equals("integer") //only thing that changed was int -> integer to match correct syntax
	...
	private Statement statement() {
		...
		if (token.getValue().equals(";")) { // Skip
			...
			s = ifStatement();
		else if (token.getValue().equals("while")) { //while TO BE COMPLETED
			s = whileStatement();
		} else if (token.getType().equals("Identifier")) { // Assignment TO BE COMPLETED
			s = assignment();
		} else
